...
namespace Practice_Linq
{
    public class Program
    {
        ...
        {
            // Query 8: Матчі Євро-2012 в Україні з новими властивостями.

            var selectedGames = games
                .Where(g =>
                    g.Tournament.Contains("UEFA Euro") &&
                    g.Country == "Ukraine" &&
                    g.Date.Year == 2012
                )
                .Select(g => new
            ...
            {
                Console.WriteLine(
                    $"{g.MatchYear} | {g.Team1} - {g.Team2} | Goals: {g.Goals}"
                );
            }
        }
        ...
        {
            // Query 9: Матчі UEFA Nations League 2023 з результатом гри.

            var selectedGames = games
                .Where(g =>
                    g.Tournament == "UEFA Nations League" &&
                    g.Date.Year == 2023
                )
                .Select(g => new
                {
        ...
        {
            // Query 13: ТОП-3 країни для проведення матчів на нейтральному полі.
            ...
            foreach (var g in selectedGames)
            {
                Console.WriteLine($"{g.Country} | {g.Count}");
            }
        }
        ...
        {
            // Query 14: ТОП-5 турнірів за середньою результативністю.
            ...
            var selectedGames = games
                ...
                {
                    Tournament = g.Key,
                    AvgGoals = g.Average(x => x.Home_score + x.Away_score)
                })
            ...
            foreach (var g in selectedGames)
            {
                Console.WriteLine($"{g.Tournament} | {g.AvgGoals:F2}");
            }
        }
